...
final class HtmlWriter
{
     ...
     */
    public function write(array $payload, string $outputPath, string $mermaid, ?string $svgFile = null): string
    {
        if (!is_dir($outputPath)) {
            mkdir($outputPath, 0777, true);
        }

        $graphHtml = $this->graphHtml($mermaid, $svgFile);
        $html = '<!doctype html><html><head><meta charset="utf-8"><title>Project Map</title>'
            . '<style>body{font-family:Arial,sans-serif;margin:32px;color:#111827}.diagram{width:100%;min-height:520px;overflow:auto;border:1px solid #d1d5db;padding:16px;background:#fff}.diagram svg{max-width:none}table{border-collapse:collapse;width:100%;margin:12px 0 28px}td,th{border:1px solid #d1d5db;padding:8px;text-align:left}code{background:#f3f4f6;padding:2px 4px}</style>'
            ...
            . '<h2>Graph</h2><div class="diagram">' . $graphHtml . '</div>'
            . '<p><a href="project-map.svg">project-map.svg</a> · <a href="project-map.dot">project-map.dot</a> · <a href="project-map.json">project-map.json</a></p>'
    ...
    private function graphHtml(string $mermaid, ?string $svgFile): string
    {
        if ($svgFile !== null && is_file($svgFile)) {
        ...
        return '<pre class="mermaid">' . $this->esc($mermaid) . '</pre>';
    }
